...
Game::Game(const std::string &playerName, int startGold, int startLives,
    ...
    : playerName_(playerName), castlePos_(Position(0, 0)), gold_(startGold),
      lives_(startLives), wave_(0), score_(0),
...
bool Game::isValidPosition(const Position &pos) {
  return pos.getX() >= 0 && pos.getX() < GRID_W && pos.getY() >= 0 &&
         pos.getY() < GRID_H;
}
...
void Game::placeTower(std::unique_ptr<Tower> tower) {
  ...
  gold_ -= tower->getCost();
  std::cout << "[GAME] Built " << tower->getName() << " (" << tower->getSymbol()
            << ") at " << tower->getPosition()
...
void Game::addEnemy(const Enemy &enemy) { enemies_.push_back(enemy); }
...
void Game::startNewWave(int numEnemies) {
  ...
  for (int i = 1; i <= numEnemies; ++i) {
    // Positions and stats come from the loaded wave config (no hardcoded
    // values)
    ...
    std::string name =
        "Goblin_" + std::to_string(wave_) + "_" + std::to_string(i);
...
int Game::findNearestEnemyIndex(const TowerSlot &ts) const {
  ...
  for (size_t i = 0; i < enemies_.size(); ++i) {
    ...
    if (ts.canAttack(enemies_[i])) {
      double dist =
          ts.get().getPosition().distanceTo(enemies_[i].getPosition());
      if (dist < minDist) {
        minDist = dist;
...
int Game::simulateTurn() {
  ...
  // Towers attack through virtual dispatch (TowerSlot → Tower* →
  // computeDamage())
  for (auto &slot : towers_) {
    int idx = findNearestEnemyIndex(slot);
    if (idx != -1) {
      bool killed = slot.attack(enemies_[idx]);
      std::cout << "  " << slot.get().getName() << " fired at "
                << enemies_[idx].getName()
                << " (HP left: " << enemies_[idx].getHealth() << ")\n";
      // Apply freeze effect if this is a FreezeTower (dynamic_cast in Game
      // context)
      ...
      if (killed) {
        ...
        std::cout << "  *** " << enemies_[idx].getName()
                  << " DESTROYED! Earned +" << enemies_[idx].getReward()
                  << " gold! ***\n";
      }
    }
  }
  ...
  for (auto &enemy : enemies_) {
    if (!enemy.isAlive())
      continue;
    enemy.tickFreeze(); // reduce frozen turns, restore speed if counter reaches
                        // 0
...
void Game::drawGrid() const {
  ...
  std::cout << "Legend: C=Castle, A=Archer, K=Cannon, M=Mage, Z=Freeze, "
               "E=Enemy, .=Empty\n";
}
